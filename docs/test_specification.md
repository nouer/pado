...
| E2E-INIT-005 | 起動時スピナー | ページアクセス直後 | ローディングスピナーが表示され、初期化完了後に消失する |
| E2E-INIT-006 | 起動時間計測 | ページアクセス | コンソールに `[Pado] init: XXX ms` が出力される |
...
| E2E-PTR-007 | 取引先詳細表示 | 取引先カード「詳細」ボタン→詳細オーバーレイ | 読み取り専用の詳細情報が表示される |
| E2E-PTR-008 | 詳細から編集遷移 | 詳細オーバーレイ「編集」ボタン | 詳細が閉じ、編集フォームが開く |
...
| E2E-ITM-005 | 品目詳細表示 | 品目行「詳細」ボタン→詳細オーバーレイ | 読み取り専用の詳細情報が表示される |
| E2E-ITM-006 | 詳細から編集遷移 | 詳細オーバーレイ「編集」ボタン | 詳細が閉じ、編集フォームが開く |
...
| E2E-PRT-003 | 帳票詳細から印刷 | 帳票詳細オーバーレイ「印刷」ボタン | 詳細が閉じ、印刷用レイアウトが表示される |

### 3.10 お知らせ通知

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| E2E-NTF-001 | お知らせ通知表示 | 未読のお知らせがある状態でアプリ起動 | クリック可能なトーストが表示される |
| E2E-NTF-002 | お知らせページ遷移 | トーストをクリック | notify.html が別タブで開き、既読化される |
| E2E-NTF-003 | 通知無効化 | 設定の「お知らせを通知する」を無効化 | アプリ起動時に通知が表示されない |
