...
| E2E-PTR-009 | 電話番号不正バリデーション | 電話番号に不正値を入力して保存 | エラーメッセージが表示される |
| E2E-PTR-010 | メール不正バリデーション | メールに不正値を入力して保存 | エラーメッセージが表示される |
| E2E-PTR-011 | 郵便番号不正バリデーション | 郵便番号に不正値を入力して保存 | エラーメッセージが表示される |
| E2E-PTR-012 | ふりがなカタカナバリデーション | ふりがなにカタカナを入力して保存 | エラーメッセージが表示される |
| E2E-PTR-013 | 登録番号桁不足バリデーション | 登録番号に桁不足の値を入力して保存 | エラーメッセージが表示される |
| E2E-PTR-014 | ふりがな検索 | ふりがなで取引先を検索 | 一致する取引先のみ表示される |
...
| E2E-ITM-007 | 負の単価バリデーション | 品目の単価に負の値を設定して保存 | エラーメッセージが表示される |
| E2E-ITM-008 | 小数単価バリデーション | 品目の単価に小数値を設定して保存 | エラーメッセージが表示される |
...
| E2E-DOC-012 | ステータス変更 | 帳票編集→ステータスを「発行済」に変更→保存 | ステータスバッジが更新される |
| E2E-DOC-013 | ステータスフィルター | ステータスフィルターで絞り込み | フィルタ条件に一致する帳票のみ表示される |
| E2E-DOC-014 | 混合税率帳票 | 10%と8%の品目を含む帳票を保存→再表示 | 税率設定が維持されている |
| E2E-DOC-015 | 数量0の明細行 | 数量0・単価有の明細行を入力 | 金額が0になる |
| E2E-DOC-016 | 単価0の明細行 | 数量有・単価0の明細行を入力 | 金額が0になる |
...
| E2E-CNV-005 | 見積書→発注書変換 | 見積書→変換ボタン→発注書選択 | 発注書エディタが開き明細がコピーされる |
| E2E-CNV-006 | 見積書→納品書変換 | 見積書→変換ボタン→納品書選択 | 納品書エディタが開き明細がコピーされる |
| E2E-CNV-007 | 請求書→売上伝票変換 | 請求書→変換ボタン→売上伝票選択 | 売上伝票エディタが開き明細がコピーされる |
...
| E2E-SET-005 | 帳票番号プレフィックス変更 | 請求書のプレフィックスをBILLに変更 | プレビューが更新される |
| E2E-SET-006 | 区切り文字変更 | 帳票番号の区切り文字を/に変更 | プレビューが更新される |
| E2E-SET-007 | 年度トグルOFF | 帳票番号の年度トグルをOFFに変更 | プレビューから年度が除外される |
| E2E-SET-008 | サブタブ表示トグル | 仕入伝票サブタブを非表示→再表示 | サブタブの表示が切り替わる |
| E2E-SET-009 | 日付形式トグル | 日付形式を和暦に切り替え | 設定が永続化される |
| E2E-SET-010 | デフォルト帳票タイプ変更 | デフォルト帳票タイプを請求書に変更 | 設定が永続化される |
| E2E-SET-011 | 設定のリロード後永続化 | 設定保存→リロード | 設定値が保持されている |
...
| E2E-DIO-006 | エクスポートファイル検証 | エクスポートデータのJSON構造確認 | partners/items/documents配列と必須フィールドが存在する |
| E2E-DIO-007 | インポートマージ動作 | 重複IDを含むデータをインポート | 既存レコードが上書きマージされる |
...
テスト対象: `usecases.e2e.test.js`（ワークフロー結合テスト）
テスト環境: Docker コンテナ内で Puppeteer を実行
実行コマンド: `docker compose run --rm pado-test npm run test:usecases`

**特記事項:**
- 既存の `npm test` には含まれず、独立したコマンドで実行する
- UC1〜UC8 は状態を引き継いで逐次実行される（beforeAll で全データ削除→空の状態からUC1開始）
- 既存 E2E テスト（e2e.test.js）とは完全分離

### 4.1 UC1: 開業準備 — 初期セットアップ

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| UC-1-01 | 自社情報設定 | 設定タブ→自社情報入力→保存 | 事業者名・住所・電話等が保存される |
| UC-1-02 | 税設定 | 端数処理・計算方式を設定→保存 | 税設定が保存される |
| UC-1-03 | 表示設定 | デフォルト帳票・日付形式・角印等を設定→保存 | 表示設定が保存される |
| UC-1-04 | 帳票番号プレフィックス確認 | 帳票番号設定セクションを確認 | QT, PO, INV, DN, SS, PS, RC が表示 |
| UC-1-05 | タブ表示トグル | 仕入伝票を非表示→再表示 | サブタブの表示/非表示が切り替わる |

### 4.2 UC2: 取引先と品目の登録

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| UC-2-01 | 得意先登録 | 株式会社ミドリ商事を登録 | 一覧に表示・P0001自動採番 |
| UC-2-02 | 仕入先登録 | デザインツール商社を登録 | 一覧に表示・P0002自動採番 |
| UC-2-03 | 品目登録（3件） | Webサイト制作・ロゴデザイン・イラスト素材集 | I0001〜I0003自動採番 |

### 4.3 UC3: 見積書の作成と確認

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| UC-3-01 | 見積書作成 | ミドリ商事向け・3明細行 | 保存成功・QT番号自動採番 |
| UC-3-02 | 品目自動入力 | 品目ドロップダウンから選択 | 単価が自動入力される |
| UC-3-03 | 明細行追加・削除 | 4行目追加→削除 | 行数が正しく変動 |
| UC-3-04 | 税額自動計算 | 標準税率+軽減税率の混在 | 小計¥510,000、合計¥560,000 |
| UC-3-05 | 有効期限デフォルト | 見積書作成時 | 約30日後が自動設定 |
| UC-3-06 | 帳票詳細表示 | 詳細ボタン→オーバーレイ | 保存内容が読み取り確認できる |
| UC-3-07 | 印刷プレビュー | 印刷ボタン | 角印・登録番号が表示される |

### 4.4 UC4: 受注から入金まで

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| UC-4-01 | 見積→請求変換 | 変換ボタン→請求書選択 | 明細コピー・変換元リンク表示 |
| UC-4-02 | 請求書印刷 | 印刷ボタン | 振込先・登録番号が含まれる |
| UC-4-03 | 請求→領収変換 | 変換ボタン→領収書選択 | 但し書き入力可・印紙税表示 |
| UC-4-04 | 領収書印刷 | 印刷ボタン | 専用レイアウト・印紙欄表示 |
| UC-4-05 | 請求→売上伝票変換 | 変換ボタン→売上伝票選択 | 正常に変換・保存 |

### 4.5 UC5: 仕入業務

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| UC-5-01 | 発注書作成 | デザインツール商社向け | 保存成功 |
| UC-5-02 | 発注→納品変換 | 変換ボタン→納品書選択 | 正常に変換・保存 |
| UC-5-03 | 納品→売上伝票変換 | 変換ボタン→売上伝票選択 | 正常に変換・保存 |
| UC-5-04 | 発注→仕入伝票変換 | 変換ボタン→仕入伝票選択 | 正常に変換・保存 |
| UC-5-05 | 見積→発注変換 | 新規見積書→発注書変換 | 正常に変換・保存 |

### 4.6 UC6: 帳票の検索と日常管理

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| UC-6-01 | 帳票番号検索 | "QT"で検索 | 見積書が絞り込まれる |
| UC-6-02 | 取引先名検索 | "ミドリ"で検索 | 該当帳票が表示 |
| UC-6-03 | ステータスフィルター | "発行済"選択 | 該当帳票のみ表示 |
| UC-6-04 | 帳票編集 | ステータス変更→保存 | 変更が反映される |
| UC-6-05 | 帳票削除 | 削除→確認 | 一覧から消える |
| UC-6-06 | サブタブ巡回 | 7タブすべて切替 | 全タブ表示可能 |
| UC-6-07 | 売上伝票直接作成→領収変換 | 新規作成→変換 | 正常に作成・変換 |

### 4.7 UC7: マスタデータの活用

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| UC-7-01 | 取引先検索 | "ミドリ"で検索 | ミドリ商事のみ表示 |
| UC-7-02 | 取引先区分フィルター | "得意先"選択 | 得意先のみ表示 |
| UC-7-03 | 取引先詳細表示 | 詳細ボタン→オーバーレイ | 読み取り専用で表示 |
| UC-7-04 | 取引先編集 | 電話番号追加→保存 | 変更が反映される |
| UC-7-05 | 品目検索 | "ロゴ"で検索 | ロゴデザインが表示 |
| UC-7-06 | 品目詳細表示 | 詳細ボタン→オーバーレイ | 読み取り専用で表示 |
| UC-7-07 | 品目編集 | 単価変更→保存 | 変更が反映される |
| UC-7-08 | 品目削除 | イラスト素材集→削除 | 一覧から消える |
| UC-7-09 | 使用中取引先の削除拒否 | デザインツール商社→削除 | 使用中エラーが表示され削除されない |
| UC-7-10 | 未使用取引先の削除 | テスト用取引先→登録→削除 | 一覧から消える |

### 4.8 UC8: データ保全

| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| UC-8-01 | エクスポート | エクスポートボタン | JSONファイルダウンロード |
| UC-8-02 | 全データ削除 | 削除→確認 | 全タブが空になる |
| UC-8-03 | インポート復元 | バックアップファイルインポート | 取引先・品目・帳票が復元 |
